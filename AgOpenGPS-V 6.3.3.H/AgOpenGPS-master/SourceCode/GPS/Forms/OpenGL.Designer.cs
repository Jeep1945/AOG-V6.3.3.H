...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
        ...
        double FlagDistanceToVehicleDou;
        string FlagDistanceToVehicle;
        ...
        private void oglMain_Paint(object sender, PaintEventArgs e)
        {
                ...
                if (isGPSPositionInitialized)
                {
                    ...
                    if (bnd.bndList.Count > 0 || bnd.isBndBeingMade == true)
                    {
                        ...
                        //if (yt.isYouTurnBtnOn && !ct.isContourBtnOn)
                        {
                            ...
                            //for (int i = 0; i < bnd.bndList.turnLine.Count; i++)
                            //{
                            bnd.bndList[0].turnLine.DrawPolygon();
                            //}
                    ...
                    if (isLightbarOn)
                    {
                        if (!isLightBarNotSteerBar) DrawSteerBarText();
                        else DrawLightBarText();
                    }
            ...
            else
            {
                ...
                font.DrawText(edge, oglMain.Height - 60, "<-- AgIO ?");  // -80 ################################################
        ...
        private void oglBack_Paint(object sender, PaintEventArgs e)
        {
            ...
            if ((bnd.bndList.Count > 0) && (!tool.isSectionOffWhenOut))
            {
                ////draw the bnd line 
        ...
        private void DrawUTurnBtn()
        {
            ...
            //###################################################
            String hede;
            hede = (yt.rowSkipsWidth - 1).ToString();
            ...
            GL.Begin(PrimitiveType.Quads);              // Build Quad From A Triangle Strip  Draws the turnarrow
            ...
            font.DrawText(360, 25, "skip " + hede, 0.9);
            GL.End();
        ...
        private void DrawSteerCircle()
        {
            ...
            int bottomSide = oglMain.Height - 70;  //30  ##################################

            //draw the clock
            GL.End();
            //GL.Color4(0.9752f, 0.80f, 0.3f, 0.98); // yellow
            GL.Color4(0.0f, 1.00f, 0.0f, 0.98);   // neon green
            //GL.Color4(0.9752f, 0.00f, 0.0f, 0.98);   //red
            font.DrawText(center - 100, oglMain.Height - 50, DateTime.Now.ToString("HH:mm:ss"), 1.2); // hight 110  ##################################
            ...
            GL.Begin(PrimitiveType.Quads);              // Build the line of roll
            {
            ...
            if ((ahrs.imuRoll != 88888))   // value of roll 
            {
                string head = Math.Round(ahrs.imuRoll, 1).ToString();
                font.DrawText((int)(((head.Length) * -7)), -35, head, 1.0);
        ...
        private void DrawFlags()
        {
            ...
            {
                GL.PointSize(15.0f);  //  flagpoint size #######################################
                GL.Begin(PrimitiveType.Points);
                string flagColor = "&";
                ...
                //  flagpoint distance #######################################
                if ((FlagAlarm) && (flagPts.Count != 0))
                {
                    if (isMetric)
                        FlagDistanceToVehicle = glm.Distance(pn.fix,
                            flagPts[f].easting, flagPts[f].northing).ToString("N2");// + "m";
                    else FlagDistanceToVehicle = (glm.Distance(pn.fix,
                        flagPts[f].easting, flagPts[f].northing) * glm.m2ft).ToString("N2");// + "m";

                    //Console.WriteLine(FlagDistanceToVehicle);

                    FlagDistanceToVehicleDou = Convert.ToDouble(FlagDistanceToVehicle); // = FlagDistanceToVehicleDou;

                    if (FlagDistanceToVehicleDou < 15)   // sound for flag
                    {
                        sounds.sndAutoSteerOn.Play();
                    }
                    else sounds.sndAutoSteerOn.Stop();

                    //draws the flag with distance
                    if (FlagDistanceToVehicleDou < 30)
                        font.DrawText3D(flagPts[f].easting, flagPts[f].northing, flagPts[f].notes + ", " + FlagDistanceToVehicle + "m");
                    else font.DrawText3D(flagPts[f].easting, flagPts[f].northing, flagColor + flagPts[f].notes);  //draws the flag
                                                                                                                  //  flagpoint distance #######################################
                }
                else font.DrawText3D(flagPts[f].easting, flagPts[f].northing, flagColor + flagPts[f].notes);
            }
        ...
        private void DrawLightBar(double Width, double Height, double offlineDistance)
        {
            ...
            if ((FlagDistanceToVehicleDou < 15) && (FlagAlarm) && (flagPts.Count != 0)) // sound for flag  #########################
            {
                GL.Color3(0.9752f, 0.752f, 0.40f);
                font.DrawText(-130, 140, "Flag too close", 1.2); //   ##################################              
            }
        ...
        private void DrawLightBarText()
        {
            ...
            if (ct.isContourBtnOn || trk.idx > -1 || recPath.isDrivingRecordedPath)
            {
                ...
                int down = 65 + (int)((double)(oglMain.Height - 600) / 17); // 58
                ...
                font.DrawText(center, 2, hede, 1.2 + textSize);
        ...
        private void DrawTrackInfo()
        {
            ...
            if (trk.gArr[trk.idx].mode == 2)  // TrackMode.AB)
            {
        ...
        private void DrawCompassText()
        {
            ...
            if (Properties.Settings.Default.setGPS_headingFromWhichSource != "Dual")
        ...
        private void DrawGuidanceLineText()
        {
            if (guideLineCounter > 0)
            {
                ...
                lblCurveLineName.Text = "Line: " + lblGuidanceLine.Text;   //###########################################################
