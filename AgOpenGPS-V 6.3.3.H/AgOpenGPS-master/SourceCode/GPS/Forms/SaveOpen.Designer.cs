...
namespace AgOpenGPS
{
    ...
    public partial class FormGPS
    {
        ...
        public void FileSaveYouturnLines()
        {
            ...
            string filename = directoryName + "\\Youturnlines.txt";

            int cnt = Ytl.tracksArryt.Count;
            ...
            using (StreamWriter writer = new StreamWriter(filename, false))
            {
                try
                {
                    if (cnt > 0)
                    {
                        writer.WriteLine("$Youturnlines");

                        for (int i = 0; i < cnt; i++)
                        {
                            //write out the name
                            writer.WriteLine(Ytl.tracksArryt[i].name);


                            //write out the moveDistance
                            writer.WriteLine(Ytl.tracksArryt[i].moveDistance.ToString(CultureInfo.InvariantCulture));

                            //write out the mode
                            writer.WriteLine(Ytl.tracksArryt[i].mode.ToString(CultureInfo.InvariantCulture));

                            //write out the A_Point index
                            writer.WriteLine(Ytl.tracksArryt[i].a_point.ToString(CultureInfo.InvariantCulture));

                            //write out the points of ref line
                            int cnt2 = Ytl.tracksArryt[i].trackPtsyt.Count;

                            writer.WriteLine(cnt2.ToString(CultureInfo.InvariantCulture));
                            if (Ytl.tracksArryt[i].trackPtsyt.Count > 0)
                            {
                                for (int j = 0; j < cnt2; j++)
                                    writer.WriteLine(Math.Round(Ytl.tracksArryt[i].trackPtsyt[j].easting, 3).ToString(CultureInfo.InvariantCulture) + "," +
                                                     Math.Round(Ytl.tracksArryt[i].trackPtsyt[j].northing, 3).ToString(CultureInfo.InvariantCulture) + "," +
                                                     Math.Round(Ytl.tracksArryt[i].trackPtsyt[j].heading, 5).ToString(CultureInfo.InvariantCulture));
                            }
                        }
                    }
                    else
                ...
                {
                    WriteErrorLog("Saving Youturn lines" + er.ToString());
        ...
        public void FileLoadYouturnLines()
        {
            Ytl.tracksArryt?.Clear();
            ...
            //get the file of previous  Lines
            if ((directoryName.Length > 0) && (!Directory.Exists(directoryName)))
            { Directory.CreateDirectory(directoryName); }

            if (!File.Exists(filename))
            {
                TimedMessageBox(2000, gStr.gsFileError, "Missing Youturn lines File");
            }
            else
            {
                using (StreamReader reader = new StreamReader(filename))
                {
                    try
                    {
                        ...
                        while (!reader.EndOfStream)
                        {
                            ...
                            Ytl.tracksArryt.Add(new CYouTurnPath());
                            Ytl.idxyt = Ytl.tracksArryt.Count - 1;

                            //read header $CurveLine
                            Ytl.tracksArryt[Ytl.idxyt].name = reader.ReadLine();

                            line = reader.ReadLine();
                            Ytl.tracksArryt[Ytl.idxyt].moveDistance = double.Parse(line, CultureInfo.InvariantCulture);

                            line = reader.ReadLine();
                            Ytl.tracksArryt[Ytl.idxyt].mode = int.Parse(line, CultureInfo.InvariantCulture);

                            line = reader.ReadLine();
                            Ytl.tracksArryt[Ytl.idxyt].a_point = int.Parse(line, CultureInfo.InvariantCulture);
                            ...
                            if (numPoints > 3)
                            {
                                Ytl.tracksArryt[Ytl.idxyt].trackPtsyt?.Clear();
                                ...
                                for (int i = 0; i < numPoints; i++)
                                {
                                    ...
                                    Ytl.tracksArryt[Ytl.idxyt].trackPtsyt.Add(vecPt);
                                }
                            }
                            else
                            {
                                if (Ytl.tracksArryt.Count > 0)
                                {
                                    Ytl.tracksArryt.RemoveAt(Ytl.idxyt);
                                }
                            }
                        }
                    }
                    ...
                    catch (Exception er)
                    {
                        ...
                        var form = new FormTimedMessage(2000, "Youturnlines Error", "Lines Deleted");
                        form.Show(this);
                        ...
                        filename = directoryName + "\\Youturnlines.txt";
                        ...
                        WriteErrorLog("Load Youturn lines" + er.ToString());
                    }
                }
            }
        ...
        public void FileDeletePatternTracks()
        {
            ...
            for (int iPa = 0; iPa < trk.gArr.Count; iPa++)
            {
                ...
                if (nam.Length > 2 && nam.Substring(0, 4) == "&Pa ")
                {
                        ...
                        if (points > 0)
                        {
                            ...
                            for (int v = 0; v < points; v++)
                            {
                                ...
                                else
                                {
                                    ...
                                    sound = int.Parse(words[6]);
                                    notes = "";
                                }

                                CFlag flagPt = new CFlag(lat, longi, east, nort, head, color, ID, sound, notes);
                                flagPts.Add(flagPt);
                            }
                        }
                    }
        ...
        public void FileEreaseSCbyHanddone()  //by Husch
        {
            /*          string fileAndDirectory = "";
                      string line = "";
                      ...
                      {
                          //Console.WriteLine("Sie wollen eine pseudo SC löschen : 2");
                          bool isv3 = false;
                          using (StreamReader reader = new StreamReader(fileAndDirectory))
                          {
                              try
                              {
                                  ...
                                  while (!reader.EndOfStream)
                                  {
                                      ...
                                      if (!line.Contains(","))
                                      {
                                          ...
                                          if (line.Contains("222,77,111"))
                                          {

                                              Console.WriteLine("Sie wollen eine pseudo SC löschen : 3");
                                              Console.WriteLine("line : " + line);
                                              isv3 = true;
                                              break;
                                          }
                                      ...
                                      for (int v = 0; v < verts; v++)
                                      {
                                          triStrip[0].patchList.RemoveAt(verts);
        ...
        //Create Tool Track curve file
        public void FileCreateToolTrackcurve()
        {
            //get the directory and make sure it exists, create if not
            string dirField = fieldsDirectory + currentFieldDirectory + "\\";
            ...
            string myFileName = "ToolTrackcurve.txt";

            //write out the file
            using (StreamWriter writer = new StreamWriter(dirField + myFileName))
            {
                //write paths # of sections
                writer.WriteLine("$ToolTrackcurve");
                writer.WriteLine("0");
            }
        }

        //save the recorded Tool Track curve
        public void FileSaveToolTrackcurve(string name = "ToolTrackcurve.Txt")
        {
            //get the directory and make sure it exists, create if not
            string dirField = fieldsDirectory + currentFieldDirectory + "\\";
            ...
            {/*
                ...
                writer.WriteLine("$ToolTrackcurve");
                if (tooltrk.gToolArr.Count > 0)
                {
                    writer.WriteLine(tooltrk.gToolArr.Count.ToString(CultureInfo.InvariantCulture));  // how many lines

                    for (int i = 0; i < tooltrk.gToolArr.Count; i++)
                    {
                        writer.WriteLine(tooltrk.gToolArr[i].nameAB);
        ...
        //load Tool Track curve.txt
        public void FileLoadToolTrackcurve()
        {
            string line;
            //Recorded Path
            string fileAndDirectory = fieldsDirectory + currentFieldDirectory + "\\ToolTrackcurve.txt";
            if (File.Exists(fileAndDirectory))
            {
                using (StreamReader reader = new StreamReader(fileAndDirectory))
                {
                    try
                    {
                        ...
                        while (!reader.EndOfStream)
                        {
                            ...
                            {
                                tooltrk.gToolArr.Add(new CToolTrk());
                                tooltrk.gToolArr[i].nameAB = reader.ReadLine();
                                line = reader.ReadLine();
                                tooltrk.gToolArr[i].ToolOffset = int.Parse(line);
                                line = reader.ReadLine();
                                ...
                                for (int v = 0; v < numPoints; v++)
                                {
                                    line = reader.ReadLine();
                                    string[] words = line.Split(',');
                                    vecRoll vecPt = new vecRoll(double.Parse(words[0], CultureInfo.InvariantCulture),
                                        double.Parse(words[1], CultureInfo.InvariantCulture),
                                        double.Parse(words[2], CultureInfo.InvariantCulture),
                                        double.Parse(words[3], CultureInfo.InvariantCulture));
                    ...
                    {
                        //mf.TimedMessageBox(3000, " loading Tool Trackcurve failed ");
                        WriteErrorLog("Load Tool Trackcurve" + e.ToString());
                    }
                ...
                try
                {
                    ...
                    for (int i = 0; i < count2; i++)
                    {
                        writer.WriteLine(
                            ...
                            flagPts[i].soundflag.ToString(CultureInfo.InvariantCulture) + "," +
                            flagPts[i].notes);
                    }
                }
