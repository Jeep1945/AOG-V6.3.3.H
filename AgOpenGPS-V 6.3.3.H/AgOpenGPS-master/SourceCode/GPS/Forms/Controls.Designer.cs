...
using AgOpenGPS.Forms.Guidance;
    ...
    public partial class FormGPS : Form
    {
        ...
        private void btnAutoSteer_Click(object sender, EventArgs e)
        {
            ...
            if (isBtnAutoSteerOn)
            {
                ...
                btnChangeMappingColor.Image = AgOpenGPS.Properties.Resources.SectionMapping;
            ...
            else
            {
                ...
                {
                    btnChangeMappingColor.Image = AgOpenGPS.Properties.Resources.ABSmooth;
                    btnChangeMappingColor.Text = "";
                }
        ...
        private void UpDateYouturn()
        {
            if (yt.isABDirectionYouturn)
            {
                yt.isYouTurnRight = yt.isYouTurnRightA;
                yt.rowSkipsWidth = yt.rowSkipsWidth1;
            }
            else
            {
                yt.isYouTurnRight = yt.isYouTurnRightB;
                yt.rowSkipsWidth = yt.rowSkipsWidth2;
            }
        }
        ...
        private void btnBuildTracks_nbutton()
        {
            //if contour is on, turn it off
            if (ct.isContourBtnOn) { if (ct.isContourBtnOn) btnContour.PerformClick(); }
        ...
        private void tramLinesMenuField_Click(object sender, EventArgs e)
        {
            ...
            //Form form99 = new FormTramLines(this, trk.gArr[trk.idx].mode != (int)TrackMode.AB);
            form99.Show(this);

        }
        private void tramsMultiMenuField_Click(object sender, EventArgs e)
        {
            if (ct.isContourBtnOn) btnContour.PerformClick();
        ...
        {
            boundariesToolStripMenuItem1();
        }
        public void boundariesToolStripMenuItem1()
        {
            if ((isJobStarted) || ((!isJobStarted) && (isSimPosfixed)))
            {
                ...
                if ((diaRes == DialogResult.Yes) || (isSimPosfixed))
                {
        ...
        public bool FlagAlarm = true;

        private void btnFlag_Click(object sender, EventArgs e)
        {
            nextflag = flagPts.Count + 1;

            double Distance_Flag_pivot = 6;
            if (nextflag == 1)
            {
                double lati, longi, northi, easti;
                northi = pn.fix.northing + (Math.Cos(fixHeading) * Distance_Flag_pivot);
                easti = pn.fix.easting + (Math.Sin(fixHeading) * Distance_Flag_pivot);
                pn.ConvertLocalToWGS84(northi, easti, out lati, out longi);
                CFlag flagPt = new CFlag(lati, longi, easti, northi,
                    fixHeading, flagColor, nextflag, 0, nextflag.ToString());
                flagPts.Add(flagPt);
                FileSaveFlags();
            }
            ...
            Console.Write("nextflag control "); Console.WriteLine(nextflag);

            if (flagPts.Count > 0)
            {
                flagNumberPicked = 1; // nextflag; ###########################  
                Form form = new FormFlags(this);
                form.Show(this);
            }
        }
        ...
        private void btnResetToolHeading_Click(object sender, EventArgs e)
        {
            ...
            // Tool in work
            Form fc = Application.OpenForms["FormToolTracks"];
            ...
            Form form = new FormToolTracks(this);
            form.Show(this);
        ...
        private void btnYouSkipEnable_Click(object sender, EventArgs e)  // ################## shows YouTurnskip and direction
        {
        ...
        public void Fenster5()    // ################## shows YouTurnLine form to create youturnline
        {
            using (var form5 = new FormYouTurnAch(this))
        ...
        private void boundaryToolToolStripMenu_Click(object sender, EventArgs e)
        {
            if (isJobStarted)
            {
                ...
                using (var form = new FormBoundaryAche(this))
                {
                    form.ShowDialog(this);
                }
            }
        ...
        private void toolContourMenue_Click(object sender, EventArgs e)
        {
            toolContourMenu();
        }

        private void toolContourMenu()
        {
            if ((trk.gArr.Count > 1) && (bnd.bndList[0].fenceLine.Count > 0))
            {
                using (var form = new FormContour(this))
                {
                    form.ShowDialog(this);
                }
                PanelUpdateRightAndBottom();
        ...
        private void SmoothABtoolStripMenu1()
        {
            if ((isJobStarted && trk.idx > -1) && (trk.gArr[trk.idx].mode == (int)TrackMode.Curve))
            {
        ...
        public void toolStripAreYouSure()
        {
            // ###############################
            if (isJobStarted)
            {
                if (autoBtnState == btnStates.Off && manualBtnState == btnStates.Off)
                {
                    ...
                    if ((result3 == DialogResult.Yes) || (DialogResult == DialogResult.Yes))
                    {
                        //FileCreateElevation();
