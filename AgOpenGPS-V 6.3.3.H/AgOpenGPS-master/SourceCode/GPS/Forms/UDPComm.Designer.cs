        ...
        private void ReceiveFromAgIO(byte[] data)
        {
            if (data.Length > 4 && data[0] == 0x80 && data[1] == 0x81)
            {
                ...
                switch (data[3])
                {
                    ...
                    case 209:  
                        {
                            double isdata_Refresh = (long)(data[5]);
                            //if (isData_Refresh == 1)  Console.WriteLine("Hello   ");
                            //else Console.WriteLine("Hello  1684861641 ");
                            isData_Refresh = (Int16)isdata_Refresh;
                            break;
                        }
                    case 215:  // Tool GPS Position in degrees###################################################
                        {
                            double LatTool = (long)(data[5]);
                            LatTool += (long)(data[6] << 8);
                            LatTool += (long)(data[7] << 16);
                            LatTool += (long)(data[8] << 24);
                            double LongTool = (long)(data[9]);
                            LongTool += (long)(data[10] << 8);
                            LongTool += (long)(data[11] << 16);
                            LongTool += (long)(data[12] << 24);
                            LatTool /= 10000000;
                            LongTool /= 10000000;
                            ...
                            String LatToolString = Convert.ToString(LatTool);
                            String LongToolString = Convert.ToString(LongTool);

                            int decim = 2;
                            if (LatTool < 10) decim = 1;
                            double.TryParse(LatToolString.Substring(0, decim), NumberStyles.Float, CultureInfo.InvariantCulture, out LatTool);
                            double.TryParse(LatToolString.Substring(decim), NumberStyles.Float, CultureInfo.InvariantCulture, out double temp);
                            //temp *= 0.01666666666667;
                            temp *= 1.6666666666666667;
                            LatTool += temp;
                            if (LatTool < 0)
                                NothSouth = "S";
                            decim = 2;
                            ...
                            //temp *= 0.01666666666667;
                            temp1 *= 1.6666666666666667;
                            LongTool += temp1;
                            if (LongTool < 0)
                                EastWest = "W";

                            pn.ConvertWGS84ToLocal(LatTool, LongTool, out pn.ToolLatitude, out pn.ToolLongitude);

                            Console.WriteLine("   ");
                            Console.WriteLine("latTool   " + LatTool);
                            Console.WriteLine("LongTool  " + LongTool);
                            //Console.WriteLine("pn.ToolLatitude   " + pn.ToolLatitude);
                            //Console.WriteLine("pn.ToolLongitude  " + pn.ToolLongitude);
