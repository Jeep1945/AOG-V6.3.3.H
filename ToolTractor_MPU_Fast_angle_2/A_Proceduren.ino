...
void heading_relposned() {
  ...
  heading = headingUBX;
  if (debugmode) {
    Serial.println(" headingUBX " + String(heading, 2));
  }
  ...
  if (debugmode) {
    ...
    Serial.print (relPosValid);
    Serial.print(" carrSoln : ");  // 0 GPS //1 FLOAT // 2 RTKFIXED
  ...
  if ((Heading_MPU6050 == 1) && (Heading_Dual_MPU > 0)) {
    ...
    heading2 = heading * (1 - Heading_Dual_MPU / 10) + heading1 * (Heading_Dual_MPU / 10);
    heading = heading2;
  }
  else
  {
    if (Heading_Dual_MPU > 0) {
      heading2 = heading * (1 - Heading_Dual_MPU / 10) + headingbefore * (Heading_Dual_MPU / 10);
      heading = heading2;
    }
